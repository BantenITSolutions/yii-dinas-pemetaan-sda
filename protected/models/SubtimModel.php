 ...
 */
class SubtimModel extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return SubtimModel the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('id_menu, id_menu_admin, judul, isi, nama_file, gambar', 'required'),
			array('id_menu, id_menu_admin', 'numerical', 'integerOnly'=>true),
			array('judul', 'length', 'max'=>150),
			array('nama_file, gambar', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_subtim, id_menu, id_menu_admin, judul, isi, nama_file, gambar', 'safe', 'on'=>'search'),
		);
	}
