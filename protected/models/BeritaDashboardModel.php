 ...
 */
class BeritaDashboardModel extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return BeritaDashboardModel the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('judul, isi, jenis', 'required'),
			array('judul', 'length', 'max'=>150),
			array('tanggal', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_berita, judul, tanggal, isi, gambar, jenis, rangkuman', 'safe', 'on'=>'search'),
            array('gambar', 'file',
                    'allowEmpty' => true,
                    'types' => 'jpg,jpeg,gif',
                 ),
