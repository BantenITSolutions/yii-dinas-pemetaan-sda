 ...
 */
class KlipingModel extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return KlipingModel the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('judul, tanggal, keterangan', 'required'),
			array('judul', 'length', 'max'=>150),
			array('tanggal', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_kliping, judul, tanggal, keterangan, nama_file', 'safe', 'on'=>'search'),
		);
	}
