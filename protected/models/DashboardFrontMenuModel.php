 ...
 */
class DashboardFrontMenuModel extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return DashboardFrontMenuModel the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('menu, content, posisi', 'required'),
			array('id_parent', 'numerical', 'integerOnly'=>true),
			array('menu, posisi', 'length', 'max'=>50),
			array('url_route', 'length', 'max'=>100),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id_menu, id_parent, menu, url_route, content', 'safe', 'on'=>'search'),
		);
	}
