...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div class="rowElem nobg">
		...
		$dataProvider = new CActiveDataProvider('DashboardMenuModel');
		$iterator = new CDataProviderIterator($dataProvider);
		$i=0;
		foreach($iterator as $category) 
		{
			if($stack[$i]==$category->id_menu_admin)
			{
				echo '<label style="padding:0px; margin:0px; width:90%; float:right;" for="chk-'.$i.'">'.$category->menu.'</label>';
			   	echo '<input type="checkbox" id="chk-'.$i.'" name="access_menu_check[]" value="'.$category->id_menu_admin.'" checked> ';
				echo '<div class="fix"></div>';	
			}
			else
			{
				echo '<label style="padding:0px; margin:0px; width:90%; float:right;" for="chk-'.$i.'">'.$category->menu.'</label>';
			   	echo '<input type="checkbox" id="chk-'.$i.'" value="'.$category->id_menu_admin.'" name="access_menu_check[]"> ';
				echo '<div class="fix"></div>';	
	...
	<div class="rowElem nobg">
		...
		<div class="formRight">
			<?php $model->profile="-"; ?>
			<?php echo $form->hiddenField($model,'profile'); ?>
